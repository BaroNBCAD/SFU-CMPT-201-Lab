...
port number). IP: 127.0.0.1 Port #: 8000
2. Is it UDP or TCP? How do you know?
TCP.
+ listen() in server.c
+ write() and read() in this client.c
+ SOCK_STREAM (it's for TCP. SOCK_DGRAM is for UDP)
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? It gets data from STDIN_FILENO (our
inputs) Line of code: read(STDIN_FILENO, buf, BUF_SIZE))
4. How does the client program end? How can you tell that in the code?
The program can end by errors (Code: handle_error and different error cases)
or it can end normally by close(sfd), after successfuly read write and close.
*/
#include <arpa/inet.h>
#include <stdio.h>
#include <stdlib.h>
