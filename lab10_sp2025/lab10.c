...
(You should not need to change client)
Understanding the Client:
1. How is the client sending data to the server? What protocol?
client.c has an array, using strncpy and send a buffer through
TCP protocol. ie PORT, IP, SOCK_STREAM, socket(), write()
2. What data is the client sending to the server?
Sending each item in message[NUM_MSG] ie "Hello", "Apple", "Car", "Green", "Dog"
Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
The thread is passed to acceptor_args struct.
It has stop flag, shared list_handle, and specific list lock.
run_acceptor initialize a new non-block socket, that create MAX_CLIENTS amount
of threads and accept MAX_CLIENTS amount of new client until the slots are
occupied.
2. How are received messages stored?
They are stored as linked list data (eg list_node) with heap memory (eg malloc)
3. What does `main()` do with the received messages?
In particular, main() calls a main acceptor_thread, sends and joins threads from
run_acceptor, then calls collect_all(), which traverses the linked list, prints
messages and then free the memory. Lastly, main() returns the count sum to
confirm if all messages were received.
4. How are threads used in this sample code?
acceptor_threads, an only main thread of main(), is sent to run_acceptor,
listens and waits until enough connections and messages from non-blocking client
sockets. Inside run_acceptor, there are MAX_CLIENTS threads that connects to
individual client and collects data. These connections are client threads,
allowing the serer to handle clients simultaneously.
*/
/*
5)
- How are sockets made non-blocking?
By using fcntl(fd, F_GETFL, 0) fcntl(fd, F_SETFL, flags | O_NONBLOCK)
- What sockets are made non-blocking?
The Server Socket (from run_acceptor) and Client Sockets (from run_client)
- Why are these sockets made non-blocking? What purpose does it serve?
For Server Socket, it allows run_acceptor to check for new connections
continuosly as long as the aargs->run is still true (only set to false in main()
when expected amount of messages is met). If socket is blocking, server or
run_acceptor will stop waiting for a connection at accept(). or when main
decides to end the Server Socket, but the socket still waits for new connection
--> Freeze server

For Client Socket, it has the same reason. As soon as cargs->run is set to
false, the loop can stop. If it's blocking, when cargs->run is false, client
socket may still wait for read().
*/
